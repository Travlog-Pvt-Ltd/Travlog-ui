...
const RegisterComp = ({ changeView, closeLogin, handleGoogleLogin }) => {
  ...
  const handleOtpVerification = async () => {
    ...
    try {
      const response = await verifyOTP('/auth/verify-otp', {
        email: data.email,
        otp: otpNumber,
      });
      enqueueSnackbar(response.data.message, { variant: 'success' });
    ...
    <>
      {!otpSent && (
        <div className='register-div'>
          <div className='login-modal-heading heading3'>
            Register on Travlog
          </div>
          <input
          ...
          />
          <button onClick={handleSendOTP} className='btn submit-btn'>
            SEND OTP
          </button>
      ...
      {otpSent && !verified && (
        <div className='enter-otp-div'>
            ...
            {otp.map((item, index) => {
              return (
                <input
                  ...
                  maxLength={1}
                  value={item}
                  onKeyDown={handleOtpBackspace(index)}
          ...
          <button onClick={handleOtpVerification} className='btn submit-btn'>
            SUBMIT
          </button>
          <p>
            {data.email} &nbsp;{' '}
            <span onClick={() => setOtpSent(false)} className='link'>
              Change
            </span>
          </p>
        </div>
      ...
      {otpSent && verified && (
        <div className='enter-password'>
          ...
          <button onClick={handleRegister} className='btn submit-btn'>
            SUBMIT
          </button>
          <p>
            {data.email} &nbsp;{' '}
